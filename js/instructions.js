...
(function () {
    ...
    function renderMarkdownLines(lines) {
        ...
        function flushParagraph() {
            if (paragraph.length) {
                html.push(`<p class="body-text">${inlineMarkdown(paragraph.join(" "))}</p>`);
                paragraph = [];
            }
        }
    ...
    function markerTextClass(label) {
        return `marker-text marker-text-len-${markerLabelLength(label)}`;
    }
    ...
    function renderInstructions(markdown) {
        ...
        if (!sections.length) {
            guide.innerHTML = '<div class="instructions-error">No sections found. Use ## headings in the Markdown file.</div>';
            return;
        }
        ...
        guide.innerHTML = sections.map((section, index) => {
            ...
            return `
                <div class="${rowClasses.join(" ")}">
                    <div class="${markerClass(label, index, sections.length)}"><span class="${markerTextClass(label)}">${escapeHtml(displayMarkerLabel(label))}</span></div>
                    <article class="${cardClasses.join(" ")}">
                        ${icon ? `<div class="card-icon">${escapeHtml(icon)}</div>` : ""}
                        <div class="card-content">
                            ${isIntro ? `<h2>${inlineMarkdown(title)}</h2>` : `<div class="step-label">${escapeHtml(label === "REF" ? "Reference" : "Section " + label)}</div><div class="step-heading">${inlineMarkdown(title)}</div>`}
    ...
    async function loadInstructions() {
        const mdPath = document.body.dataset.labMd || "./fs-navigation.md";
        try {
            ...
            const guide = document.getElementById("guide-scroll");
            if (guide) {
                guide.innerHTML = `<div class="instructions-error"><strong>Could not load instructions.</strong><br>Check that ${escapeHtml(mdPath)} exists and that the page is being served from a local web server.</div>`;
            }
        }
    }
